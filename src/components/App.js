  ...
  const navigate = useNavigate()
    ...
    useEffect(() => {
      function handleTokenCheck() {
        ...
        if (token) {
          authApi.checkToken(token).then((email) => {
            if (email) {
              setLoggedIn(true)
      ...
      <div className="page">
        <Header loggedIn={loggedIn} currentEmail={currentEmail} handleSignOut={handleSignOut} />
          <Routes>
              ...
              <Route path="/sign-in" element={ <Login handleLogin={handleLogin} setUserEmail={setCurrentEmail} handleRegister={handleRegister} />} />
